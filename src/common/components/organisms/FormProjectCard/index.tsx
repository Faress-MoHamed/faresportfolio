...
import SkillComponent from "../../atoms/SkillComponent";
import { Button } from "../../atoms/button";
import { useDispatch } from "react-redux";
import { deleteProject } from "@/actions/ProjectActions";
import { editProject } from "@/utils/redux/RTK/slices/ProjectReducer";
import {
	Carousel,
	CarouselContent,
	CarouselItem,
...
// import { Button } from "../../atoms/button";
// interface FormProjectCardProp extends ProjectCardProp {
// 	onEdit: () => void;
// 	onDelete: () => void;
// }
interface ProjectCardFormProp {
	id: string;
	mediaUrls: string;
	title: string;
	description: string;
...
// onEdit,
ProjectCardFormProp) => {
	// const selector = useSelector((state) => state.project);
			...
			<div className="rounded-lg bg-card text-card-foreground flex flex-col overflow-hidden border hover:shadow-lg transition-all duration-300 ease-out h-full">
				...
				>
					...
					>
						...
						<CarouselPrevious />
						<CarouselContent>
							{JSON.parse(mediaUrls).map((media: any, idx: any) =>
				...
				<div className="flex flex-col px-2">
					<div className="space-y-1">
						...
						<time className="text-xs">Jan 2024 - Feb 2024</time>
						<div className="prose max-w-full text-pretty text-xs text-muted-foreground dark:prose-invert">
							<p>{description}</p>
						</div>
					</div>
				</div>
				<div className="text-pretty text-sm text-muted-foreground mt-auto flex flex-col px-2">
					<div className="mt-2 flex flex-wrap gap-1">
						{JSON.parse(skills).map((el: any) => (
							<SkillComponent key={el} skill={el} />
						))}
					</div>
				</div>
				<div className="flex items-center pt-2 px-2 pb-2">
					<div className="flex flex-row flex-wrap items-start gap-1">
						...
						{GitgubLink && (
							<Link target="_blank" href={GitgubLink}>
								<div className="items-center rounded-md border font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-primary text-primary-foreground shadow hover:bg-primary/80 flex gap-2 px-2 py-1 text-[10px]">
				...
				<div className="flex items-center py-2">
					<div className="flex flex-row flex-wrap items-start gap-1">
						...
						<Button onClick={() => handleDeleteProject(id)}>Delte</Button>
					</div>
				</div>
			</div>
		</>
...
export default FormProjectCard;
